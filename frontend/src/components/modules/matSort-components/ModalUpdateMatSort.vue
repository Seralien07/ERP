<template>
   <div class="modal fade" style="margin-top:32px !important;" id="updateMatSort" tabindex="-1" aria-labelledby="" aria-hidden="true">
    <div class="modal-dialog modal-md rounded-0">
        <div class="modal-content rounded-0">
            <div class="modal-header rounded-0 ag-color-2">
                <h5 class="modal-title " style="margin-left: auto;color:white">Modification materiel sortant</h5>
            ...
            <div class="modal-footer footer-bg">
                ...
                <button type="submit" name="addClient" class="btn btn-sm text-nowrap ag-color-10-hover ag-color-10" data-bs-dismiss="modal" @click="updateMatSort()">Valider</button>
            </div>
        </div>
    </div>
</div>
...
export default {
    ...
    props:['matSortDetails','allStaff','allMat'],
    data: function () {
        return {
            idMatSort: '',
            nameMater: '',
    ...
    methods: {
        updateMatSort () {
            ...
            this.$api.matSortApi.updateMatSort(this.idMatSort,params)
                .then((data) => {
                    this.notifyEvent();
        ...
        initializeInputs () {
            this.idMatSort = this.matSortDetails.id;
            this.nameMater = this.matSortDetails.nameMater;
            this.serials = this.matSortDetails.serials;
            this.quantity_sort = this.matSortDetails.quantity_sort;
            this.comments = this.matSortDetails.comments;
            this.staff = this.matSortDetails.idStaff;
            this.mat = this.matSortDetails.idMat;
        },
        notifyEvent(isSuccess = true) {
            if(isSuccess){
                this.$notify('Materiel Sortant a bien été modifié.',{
                        type: 'success',
                        showIcon: 'true',
