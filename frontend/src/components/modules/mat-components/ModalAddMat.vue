<template>
   <div class="modal fade" style="margin-top:32px !important;" id="newMat" tabindex="-1" aria-labelledby="" aria-hidden="true">
    <div class="modal-dialog modal-md rounded-0">
        <div class="modal-content rounded-0">
            <div class="modal-header rounded-0 ag-color-2">
                <h5 class="modal-title " style="margin-left: auto;color:white">Nouveau materiel</h5>
            ...
            <div class="modal-footer footer-bg">
                ...
                <button type="submit" name="addClient" class="btn btn-sm text-nowrap ag-color-10-hover ag-color-10" data-bs-dismiss="modal" @click="addNewMat()">Enregistrer</button>
            </div>
        </div>
    </div>
</div>
...
export default {
  ...
  methods: {
    addNewMat () {
        ...
        this.$api.matApi.creatMat(params)
            .then((data) => {
                this.notifyEvent();
    ...
    notifyEvent(isSuccess = true) {
        if(isSuccess){
            this.$notify('Le nouvel materiel a bien été créé.',{
                    type: 'success',
                    showIcon: 'true',
