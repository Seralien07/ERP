    ...
    <div class="modal-dialog modal-md rounded-0">
        <div class="modal-content rounded-0">
            ...
            <div class="modal-body">
                ...
                <div class="inline"></div>
                    <div class="inlineBlock">
                        <div class="line">Voulez vous vraiment suprimer le materiel du type</div>
                        <div class="line" id="client-name-to-delete" style="font-weight: bold">{{matType}}</div><span class="fs-15"> ?</span>
                    </div>
                </div>
            </div>
            <div class="modal-footer footer-bg">
                ...
                <button type="text" name="delete-client" class="btn btn-sm text-nowrap ag-color-12-hover ag-color-12" data-bs-dismiss="modal" @click="deleteMat()" >Suprimer</button>
            </div>
        </div>
    </div>
</div>
...
export default {
    ...
    methods: {
        deleteMat () {
            this.$api.matApi.deleteMat(this.matId)
                .then((data) => {
                    this.notifyEvent();
        ...
        notifyEvent(isSuccess = true) {
            if(isSuccess){
                this.$notify('Le materiel a bien été supprimer.',{
                        type: 'success',
                        showIcon: 'true',
