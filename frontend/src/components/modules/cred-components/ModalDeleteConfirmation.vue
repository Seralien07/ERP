    ...
    <div class="modal-dialog modal-md rounded-0">
        <div class="modal-content rounded-0">
            ...
            <div class="modal-body">
                ...
                <div class="inline"></div>
                    <div class="inlineBlock">
                        <div class="line">Voulez vous vraiment suprimer le credit de l'utilisatreur</div>
                        <div class="line" id="client-name-to-delete" style="font-weight: bold">{{credUsername}}</div><span class="fs-15"> ?</span>
                    </div>
                </div>
            </div>
            <div class="modal-footer footer-bg">
                ...
                <button type="text" name="delete-client" class="btn btn-sm text-nowrap ag-color-12-hover ag-color-12" data-bs-dismiss="modal" @click="deleteCred()" >Suprimer</button>
            </div>
        </div>
    </div>
</div>
...
export default {
    ...
    methods: {
        deleteCred () {
            this.$api.credApi.deleteCred(this.credId)
                .then((data) => {
                    this.notifyEvent();
        ...
        notifyEvent(isSuccess = true) {
            if(isSuccess){
                this.$notify('offre a bien été supprimer.',{
                        type: 'success',
                        showIcon: 'true',
