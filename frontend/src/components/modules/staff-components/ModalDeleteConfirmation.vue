    ...
    <div class="modal-dialog modal-md rounded-0">
        <div class="modal-content rounded-0">
            ...
            <div class="modal-body">
                ...
                <div class="inline"></div>
                    <div class="inlineBlock">
                        <div class="line">Voulez vous vraiment suprimer le personnel</div>
                        <div class="line" id="client-name-to-delete" style="font-weight: bold">{{staffName}}</div><span class="fs-15"> ?</span>
                    </div>
                </div>
            </div>
            <div class="modal-footer footer-bg">
                ...
                <button type="text" name="delete-client" class="btn btn-sm text-nowrap ag-color-12-hover ag-color-12" data-bs-dismiss="modal" @click="deleteStaff()" >Suprimer</button>
            </div>
        </div>
    </div>
</div>
...
export default {
    ...
    methods: {
        deleteStaff () {
            this.$api.staffApi.deleteStaff(this.staffId)
                .then((data) => {
                    this.notifyEvent();
        ...
        notifyEvent(isSuccess = true) {
            if(isSuccess){
                this.$notify('Le personnel a bien été supprimer.',{
                        type: 'success',
                        showIcon: 'true',
